...
import { clearOrderFields, toastMessages } from '../shared/constants';
...
export const OrderModal: React.FC<IOrderModal> = ({ toggleModal, customer }) => {
    ...
    const submit = async (e: any) => {
        ...
        if (!name) {
            return toast.error(t("Form.inCompleteMessage").replace("record", name), toastMessages.OPTION);
        } else if (!count) {
...
console.log(formData)
    ...
    return (
        <>
            {/* Main modal */}
            <div className="fixed top-0 left-0 w-full h-full flex justify-center items-center bg-gray-300 bg-opacity-70 z-50">
                <div className="bg-white rounded-lg p-4 md:p-8 min-w-[95%] md:min-w-[80%] lg:max-w-[50%]">
                    {/* Modal content */}
                    <div className="relative">
                        ...
                        <FormCreateOrder formDataModal={formData} fields={orderFields} handleChange={change} handleSubmit={submit} handleClose={toggleModal} />
                    </div>
                </div>
            </div>
        </>
...
export const FetchOrderModal: React.FC<IOrderModal> = ({ toggleModal, customer }) => {
    ...
    const orderFields = [
        { name: "id", value: formDataModal.id, placeholder: `${t("Form.id")}` },
        { name: "name", value: formDataModal.name, placeholder: `${t("Form.name")}` },
        { name: "count", value: formDataModal.count, placeholder: `${t("Form.count")}` },
        { name: "price", value: formDataModal.price, placeholder: `${t("Form.price")}` },
        { name: "extra", value: formDataModal.extra, placeholder: `${t("Form.extra")}` },
        { name: "total_price", value: formDataModal.total, placeholder: `${t("Form.total")}` },
    ]
        ...
        <>
            ...
            <div className="fixed top-0 left-0 w-full h-full flex justify-center items-center bg-gray-300 bg-opacity-70 z-50">
                <div className="bg-white rounded-lg p-4 md:p-8 min-w-[95%] md:min-w-[80%] lg:max-w-[50%]">
                    ...
                    <div className="relative">
                        {/* Modal header */}
                        <div className="flex items-center justify-between border-b  m-10">
                            ...
                            <button
                                type="button"
                                onClick={toggleModal}
                                className="text-red-400 hover:bg-red-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 flex justify-center items-center"
                                data-modal-toggle="crud-modal"
                            >
                                <svg
                                    className="w-3 h-3"
