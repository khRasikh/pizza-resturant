    ...
    <table className="min-w-full text-left text-sm font-light text-black  justify-center items-center content-center">
      ...
      <thead className="border-b bg-white font-medium dark:border-neutral-500 dark:bg-neutral-600 rounded-md">
        <tr>
          {columns.map((l) => {
            return (
              <th scope="col" key={l} className="px-3 py-2">
                {l}
              </th>
            );
          })}
          ...
          items.map((i) => {
            return (
              <tr
                ...
                className={clsx(
                  `${
                    i.KNr % 2 !== 0 ? "bg-salte-100" : "bg-white"
                  } border-b hover:bg-slate-300 hover:font-bold hover:rounded-md  text-black`
                )}
              >
                <td className="whitespace-nowrap px-3 py-2">{i.KNr}</td>
                <td className="whitespace-nowrap px-3 py-2">
                  <button
                    onClick={() => createOrderAsync(i)}
                    className="text-green-700 hover:text-black hover:font-bold"
                  >
                    {i.Name}
                  </button>
                </td>
                ...
                <td className="whitespace-nowrap px-3 py-2">
                  <div className="flex flex-row">
                    ...
                    <button className="mx-3" onClick={() => createOrderAsync(i)}>
                      <svg
                        className="w-6 h-6 text-green-800 hover:text-red-800 hover:font-bold dark:text-white"
                        aria-hidden="true"
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
                        viewBox="0 0 21 18"
    ...
    <table className="min-w-full text-sm font-light text-center">
      <thead className="border-b bg-white font-medium dark:border-neutral-500 dark:bg-neutral-600 rounded-md">
        <tr>
          {columns.map((l) => {
            return (
              <th key={l} scope="col" className="px-6 py-1">
                {l}
              </th>
            );
          })}
        ...
        {!isLoading && items.length > 0 ? (
          items.map((i) => (
            <tr
              ...
              className={clsx(
                `${
                  i.CompNum % 2 !== 0 ? "bg-neutral-100" : "bg-white"
                } text-black hover:bg-slate-300 hover:font-bold hover:rounded-md border-b dark:border-neutral-500 dark:bg-neutral-600`
              )}
            >
              ...
              <td className="whitespace-nowrap px-3 py-1">
                {" "}
                {i.JumboPreis === parseInt("0") ? (
                  <span className="text-sm text-gray-400">{t("Label.notAvailable")}</span>
                ) : (
                  `€ ${i.JumboPreis}`
                )}
              </td>
        ...
        {items.length > 0 ? (
          items.map((i, index = 0) => (
            ...
            >
              ...
              <td className="whitespace-nowrap px-4 py-2 w-fit">€ {formatNumber(Number(i.total))}</td>
            </tr>
          ))
        ) : (
...
export const NoResultFound = ({ message }: NoResultFoundProps) => {
  return (
    <div className="text-black my-2 py-2 flex justify-center items-center">
      <div className="whitespace-nowrap px-6 py-4 font-bold">{message}</div>
    </div>
  );
};

export const PaginationCustomized = ({
  totalItems,
  pageNumber,
  pageItemsSize,
  setPageItemsSize,
  ...
  return (
    <div className="flex flex-row justify-between">
      <div className="items-center flex flex-col-reverse sm:flex-row-reverse  py-3 pl-4 ">
        <div className="p-2 text-yellow-300 font-bold">{currentDateTime}</div>
      </div>
      <div className="items-center flex flex-col-reverse sm:flex-row-reverse  py-3 pl-4 ">
